...
export default class OutputList extends Component {
  ...
  copyOutputs(event) {
    let outputs = this.state.outputs.map(output => output.data)
    atom.clipboard.write(JSON.stringify(outputs))
  }
  ...
  editPlugin(output) {
    let plugin = this.state.plugins.find(plugin => plugin.name === output.plugin)
  ...
  selectPlugin(output, plugin) {
    let outputs = this.state.outputs.map(_output => {
      if (_output.key === output.key) return Object.assign({}, _output, {plugin})
      return _output
    })
    this.setState({outputs})
    this.history[this.state.historyIndex].outputs = this.state.outputs.map(output => Object.assign({}, output))
  }

  selectHistory(event) {
    let historyIndex = event.target.value
    let outputs = this.history[historyIndex].outputs.map(output => Object.assign({}, output))
  ...
  parseProps(props) {
      ...
      switch (props.status) {
        ...
        case 'stopped':
          this.history = this.history.concat({
            time: new Date(),
            outputs: this.state.outputs.map(output => Object.assign({}, output))
          })
    ...
    if (props.output) {
      let outputs = this.parseOutput(props.output).reduce((outputs, output, index) => {
        ...
        if (_index > -1) {
          Object.assign(outputs[_index], {data})
        } else {
          outputs.push({
            key,
            data,
